...
<div class="container py-4">
  <h1 class="mb-3 text-orange">Modifier l'hôtel — {{ $hotel->name }}</h1>

  <form method="POST" action="{{ route('agency.hotels.update', $hotel) }}" enctype="multipart/form-data" class="panel-cream rounded-20 p-3 p-md-4">
    @csrf
    @method('PATCH')
    <div class="row g-3">
      <div class="col-md-6">
      ...
      <div class="col-md-6">
        ...
        <input type="text" name="phone" class="form-control" value="{{ old('phone', $hotel->phone) }}" required>
      </div>
      <div class="col-md-6">
        <label class="form-label">Email</label>
        <input type="email" name="email" class="form-control" value="{{ old('email', $hotel->email) }}" required>
      </div>
      <div class="col-md-6">
        <label class="form-label">Étoiles</label>
        <input type="number" min="1" max="5" name="stars" class="form-control" value="{{ old('stars', $hotel->stars) }}" required>
      </div>
      <div class="col-md-6">
        ...
        @if($hotel->photo)
          ...
          <img src="{{ asset($hotel->photo) }}" alt="{{ $hotel->name }}" class="img-fluid rounded" style="max-height:140px;">
        @endif
      </div>
      <div class="col-md-6">
        <label class="form-label">Latitude <span class="text-muted small">(optionnel)</span></label>
        <input type="number" step="0.00000001" min="-90" max="90" name="latitude" class="form-control" value="{{ old('latitude', $hotel->latitude) }}" placeholder="Auto-remplie depuis l'adresse si vide">
      </div>
      <div class="col-md-6">
        <label class="form-label">Longitude <span class="text-muted small">(optionnel)</span></label>
        <input type="number" step="0.00000001" min="-180" max="180" name="longitude" class="form-control" value="{{ old('longitude', $hotel->longitude) }}" placeholder="Auto-remplie depuis l'adresse si vide">
      </div>
      <div class="col-md-6">
        <label class="form-label">Équipements</label>
        <div class="border rounded p-2" style="max-height:180px; overflow:auto;">
          @foreach(($amenities ?? []) as $amenity)
            @php $checked = in_array($amenity->id, old('amenities', $hotel->amenities->pluck('id')->all())); @endphp
            <div class="form-check">
              <input class="form-check-input" type="checkbox" name="amenities[]" value="{{ $amenity->id }}" id="am-{{ $amenity->id }}" @checked($checked)>
      ...
      <div class="col-md-6">
        ...
        <div class="border rounded p-2" style="max-height:180px; overflow:auto;">
          @foreach(($rules ?? []) as $rule)
            @php $checked = in_array($rule->id, old('rules', $hotel->rules->pluck('id')->all())); @endphp
            <div class="form-check">
              <input class="form-check-input" type="checkbox" name="rules[]" value="{{ $rule->id }}" id="rl-{{ $rule->id }}" @checked($checked)>
      ...
      <div class="col-12">
        ...
        @if($hotel->photos && $hotel->photos->count())
          <div class="d-flex gap-2 flex-wrap mt-2">
            @foreach($hotel->photos as $p)
              <img src="{{ asset($p->path) }}" alt="photo" class="rounded" style="height:80px; object-fit:cover;">
            @endforeach
          </div>
        @endif
      </div>
      ...
      <div class="col-12">
        ...
        <textarea name="description" rows="5" class="form-control">{{ old('description', $hotel->description) }}</textarea>
      </div>
      <div class="col-12">
